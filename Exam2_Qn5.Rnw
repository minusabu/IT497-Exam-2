...
#RawData.5$GRADEDATE<-as.Date(RawData.5$GRADEDATE, "%m/%d/%y")
@
...
ggplot(subset(RawData.5, DBA=="MCDONALD'S"), aes(CURRENTGRADE, fill=CURRENTGRADE))+ geom_bar(position=position_dodge()) + 
  labs(x="Grade", y="Number of grades", title="Grades for McDonald's")+
  theme(legend.title=element_blank()) + guides(fill=FALSE)

@
From this graph, we can see that McDonald's has recieved a lot of A grades, but at the same time, they have received a considerable number of C grades as well. Let us look at the grade distribution for Wendy's. 

<<plotwendys, echo=FALSE, message=FALSE, fig=TRUE>>=
#plot the count of each grade for Wendy's
ggplot(subset(RawData.5, DBA=="WENDY'S"), aes(CURRENTGRADE, fill=CURRENTGRADE))+ geom_bar(position=position_dodge()) + 
...
Therfore, it can be concluded from the reported inspections that \textbf{Wendy's is cleaner than McDonald's.} \\  
\end{document}